...
i_x= \sqrt (I_x/A_s)
Момент инерции в плоскости у (см^4):
I_y=I_x=(h_0*b^3-h_1*a^3)/12
Момент сопротивления в плоскости у (см куб):
W_y=W_x=(h_0*b^3-h_1*a^3)/(6*b)
Радиус инерции в плоскости у (см):
i_y= \sqrt (I_y/A_s)
'''
    txt.c1(t,50)
...
m=(100*e*A_s)/W_x
Расчетная длина элемента по оси x (м):
l_x=L*\mu_x
Расчетная длина элемента по оси y (м):
l_y=L*\mu_y
Определение гибкости по оси x:
\lambda_x=l_x/i_x*100
Определение гибкости по оси y:
\lambda_y=l_y/i_y*100
...
l\\bar_x=\lambda_x*%(R_y/E)=73.73*(2350.0/2000000.0)^0.5=2.53
Условная гибкость стрежня в плоскости y:
...
d=9.87*(1-a1+b1*l\\bar_x)+l\\bar_x^2=9.87*(1-19.01+0.09*2.53)+2.53^2=18.12
...
10.4.1 Гибкости элементов \lambda  (l/i) не должны превышать
...
'''
    ...
    if txt.numer['a_2'] < 0.5:
        ...
        'a_2=0.5\n')
        txt.c1(t)
    elif txt.numer['a_2'] > 1:
        t = ('\nПринимаем значение коэффициента альфа не более 1:\n' +
        'a_2=1\n')
        txt.c1(t)
    t ='''
