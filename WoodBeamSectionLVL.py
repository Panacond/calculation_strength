...
5.1 Расчетные сопротивления древесины сосны, ели и лиственницы европейской влажностью 
12 % для основного сочетания нагрузок (режим В согласно таблице В.1)в сооружениях 
нормального (2-го согласно приложению Г) уровня ответственности при сроке эксплуатации 
до 50 лет приведены в таблице 3. Расчетные сопротивления для других пород древесины 
устанавливают путем умножения величин, приведенных в таблице 3, на переходные 
коэффициенты, указанные в таблице 5. Расчетные сопротивления LVL из однонаправленного 
шпона приведены в таблице 4:
Расчетные сопротивления, МПа, для сортов/классов прочности LVL  (3/К35 ) (МПа):
R_i=20
а) для различных условий эксплуатации конструкций — коэффициент указанный в таблице 7 
(По таблице 1 принимаем класс эксплуатации 3):
m_b=1
б) для конструкций, эксплуатируемых при установившейся температуре воздуха до +35 °С 
коэффициент:
m_t=1
в) для конструкций, в которых напряжения в элементах, возникающие от постоянных и 
...
6.9 Расчет изгибаемых элементов,обеспеченных от потери устойчивости плоской формы 
деформирования (см.6.14 и 6.15), на прочность по нормальным напряжениям следует 
производить по формуле (МПа):
\sigma_r=M_y*10/W_tr=259.14*10/240.83=10.76
Коэффициент использования составит:
k_1=\sigma_r/R_u=10.76/12.6=0.85

5 Скалывание вдоль волокон:
а)  Скалывание вдоль волокон в плоскости листа (МПа):
R_s0=1.9
...
Смятие местное в плоскости листа поперек волокон в опорных частях (МПа):
R_s1=4.8
Расчетное сопротивление  скалыванию составит (МПа):
R_sm=R_s1*m_b*m_t*m_d/\gamma_h=10.0*0.9*1.0*1.0/1.0=9.0
7.1 Действующее на соединение (связь)усилие не должно превышать расчетной несущей 
способности соединения (связи)Т.
7.2 Расчетную несущую способность соединений, работающих на смятие и скалывание, 
