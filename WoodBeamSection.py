...
R_i=14
а) для различных условий эксплуатации конструкций — коэффициент указанный 
в таблице 7 (По таблице 1 принимаем класс эксплуатации 3):
m_b=1
...
m_d=1
Расчетные сопротивления, приведенные в таблицах 3,4 и 6,следует 
разделить на коэффициенты надежности по сроку службы (таблица 12):
\gamma_h=1
Расчетное сопротивление составит (МПа):
R_u=R_i*m_b*m_t*m_d/\gamma_h=14.0*0.9*1.0*1.0/1.0=12.6
6.9 Расчет изгибаемых элементов, обеспеченных от потери устойчивости плоской 
формы деформирования (см.6.14 и 6.15), на прочность по нормальным напряжениям 
следует производить по формуле (МПа):
...
в) элементы прямоугольного сечения шириной свыше 13 см при высоте сечения 
свыше 13 до 50 см (МПа):
R_s1=10
Расчетное сопротивление  скалыванию составит (МПа):
R_sm=R_s1*m_b*m_t*m_d/\gamma_h=10.0*0.9*1.0*1.0/1.0=9.0
7.1 Действующее на соединение (связь)усилие не должно превышать 
расчетной несущей способности соединения (связи)Т.
7.2 Расчетную несущую способность соединений, работающих на смятие 
и скалывание, следует определять по формулам:
