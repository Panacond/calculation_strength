...
m_d=1
Расчетные сопротивления, приведенные в таблицах 3,4 и 6, следует разделить на коэффициенты 
надежности по сроку службы (таблица 12)
\gamma_h=1
...
Гибкость элемента составит:
\lambda=(L*100)/i_min
6.2  Расчет центрально-сжатых элементов постоянного цельного сечения на устойчивость
'''
    txt.c1(t)
    if txt.numer['\lambda'] > 70:
        t='''при гибкости элемента больше 70
...
'''
    txt.c1(t)
    t='''6.2  Расчет центрально-сжатых элементов постоянного цельного сечения
